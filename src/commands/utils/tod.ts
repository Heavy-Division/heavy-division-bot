...
export const rot: CommandDefinition = {
	...
	executor: (msg) => {
		...
		const error =
			"Please specify a valid altitude (1000-10,000ft) or flight level between FL100 and FL550.";

		const flightLevelError =
			"Hint: Try using the 'Flight Level' convention for values above 10,000ft up to 50,000ft" +
			"E.g. 18000ft = FL180 ";
		...
		if (altitude >= 1000) {
			const topOfDescent = Math.floor(
				altitude * (1 / 100) * (1 / 3) * (11 / 10),
			);
