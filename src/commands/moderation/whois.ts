...
export const whois: CommandDefinition = {
	name: "whois",
	description:
		"Provides an embedded message with information about the mentioned user",
	category: CommandCategory.MODERATION,
	executor: async (msg) => {
		const query = msg.content
			.replace(/\.whois(\s|$)+/, "")
			.replace(/[@#!<>]+/g, "");
		const targetMember = query
			? await msg.guild.members.fetch(query)
			: msg.member;

		// eslint-disable-next-line eqeqeq
		const filteredRoles = targetMember.roles.cache.filter(
			(role) => role.id != msg.guild.id,
		);
		const listedRoles = filteredRoles
			.sort((a, b) => b.position - a.position)
			.map((role) => role.toString());

		const onlineStatus =
			beautifiedStatus[targetMember.presence?.status.toUpperCase()];
