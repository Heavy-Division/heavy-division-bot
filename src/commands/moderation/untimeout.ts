...
export const unTimeoutDMEmbed = (moderator: User, guild: Guild) =>
	makeEmbed({
		...
		footer: {
			...
			text: `Timeout removed by ${moderator.tag}`,
		},
	});

export const unTimeoutEmbed = (user: User) =>
	makeEmbed({
...
export const unTimeoutModLogEmbed = (moderator: User, user: User) =>
	makeEmbed({
		color: Colors.Green,
		author: {
...
const failedUnTimeoutEmbed = (user: User) =>
	makeEmbed({
...
export const untimeout: CommandDefinition = {
	...
	executor: async (msg) => {
		...
		if (args.length < 1) {
			await msg.reply(
				"You need to provide the following arguments for this command: <id>",
			);
			return;
		}
