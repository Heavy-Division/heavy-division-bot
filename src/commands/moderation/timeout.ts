...
const DMEmbed = (
	...
	makeEmbed({
		...
		footer: {
			text: `Your timeout will be lifted on ${timedOutUntil.toUTCString()}`,
		},
	});

const timeoutEmbed = (user: User, reason: string, timeoutDuration: string) =>
	makeEmbed({
...
const failedTimeoutEmbed = (user: User) =>
	makeEmbed({
...
export const timeout: CommandDefinition = {
	...
	executor: async (msg) => {
		...
		if (args.length < 3 && parseInt(args[1]) !== 0) {
			return msg.reply(
				"You need to provide the following arguments for this command: <id> <timeoutDuration> <reason>",
			);
		}
		...
		switch (timeoutArg[timeoutArg.length - 1].toLowerCase()) {
			default: {
				...
				timeoutDuration =
					parseInt(timeoutArg.replace("m", "")) *
					TimeConversions.MINUTES_TO_MILLISECONDS;
				break;
			}
			case "h": {
				timeoutDuration =
					parseInt(timeoutArg.replace("h", "")) *
					TimeConversions.HOURS_TO_MILLISECONDS;
				break;
			}
			case "d": {
				timeoutDuration =
					parseInt(timeoutArg.replace("d", "")) *
					TimeConversions.DAYS_TO_MILLISECONDS;
				break;
			}
			case "w": {
				timeoutDuration =
					parseInt(timeoutArg.replace("w", "")) *
					TimeConversions.WEEKS_TO_MILLISECONDS;
				break;
			}
		}
		...
		return targetUser.timeout(timeoutDuration, reason).then(async () => {
			...
			if (targetUser.isCommunicationDisabled()) {
					...
					await targetUser.send({
						embeds: [
							DMEmbed(
								...
								reason,
								msg.guild,
								targetUser.communicationDisabledUntil,
							),
						],
					});
				} catch (e) {
				...
				if (modLogsChannel) {
					await modLogsChannel.send({
						embeds: [
							modLogEmbed(msg.author, targetUser.user, reason, timeoutArg),
						],
					});
				}
				return setTimeout(() => {
					// Delete the command and response after 4 seconds
					timeoutResponse.delete();
					msg.delete();
				}, 4000);
			}
