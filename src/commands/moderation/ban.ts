...
const successfulBanEmbed = (user: User, reason: string) =>
    makeEmbed({
...
const failedBanEmbed = (user: User, error: any) =>
    makeEmbed({
...
const unknownUserEmbed = (userId: string) =>
    makeEmbed({
...
const dmEmbed = (formattedDate, moderator: User, reason: string) =>
    makeEmbed({
        title: "You have been banned from Heavy Division",
        fields: [
            {
                inline: false,
                name: "Moderator",
...
export const ban: CommandDefinition = {
    ...
    executor: async (msg) => {
        ...
        if (splitUp.length < 3) {
            await msg.reply(
                "You did not provide enough arguments for this command. (<id> <days> <reason>)",
            );
		...
		if (!targetUser.moderatable && msg.channel.isSendable()) {
			return msg.channel.send({ embeds: [moderatableFailEmbed] });
		}
		try {
			await targetUser.send({
				embeds: [dmEmbed(formattedDate, moderator, reason)],
			});
		} catch {
			await modLogsChannel.send({
				content: moderator.toString(),
				embeds: [noDM(targetUser.user)],
			});
		}
		...
		try {
			const user = await msg.guild.members.ban(idArg, {
				deleteMessageSeconds: deleteDaysNumber * 86400,
				reason,
			});
			...
			return msg.channel.send({
				embeds: [successfulBanEmbed(targetUser.user, reason)],
			});
		} catch (error) {
			return msg.channel.send({
				embeds: [failedBanEmbed(targetUser.user, error)],
			});
		}
	},
};
