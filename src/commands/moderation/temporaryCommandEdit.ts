...
const helpEmbed = (evokedCommand: string) =>
	makeEmbed({
		...
		description:
			"A command to manage temporary commands that consist of a Title, Content and Color. This can be used to create temporary commands for different purposes.",
		fields: [
			{
			...
			},
			{
				...
				value:
					"`.temporarycommandedit`, `.tempcommandedit`, `.tcedit`, `.tcmod`",
				inline: false,
			},
		],
	});

const successEmbed = (action: string, command: string) =>
	makeEmbed({
		title: `Temporary Command - ${action} Success - ${command}`,
		color: Colors.Green,
	});

const infoEmbed = (fields: EmbedField[], command: string) =>
	makeEmbed({
		title: `Temporary Command - Info - ${command}`,
		fields,
	});

const failedEmbed = (action: string, command: string) =>
	makeEmbed({
...
const alreadyExistsEmbed = (action: string, command: string) =>
	makeEmbed({
...
const missingInfoEmbed = (action: string, information: string) =>
	makeEmbed({
...
const noChannelEmbed = (action: string, channel: string) =>
	makeEmbed({
...
const notFoundEmbed = (action: string, command: string) =>
	makeEmbed({
...
const timestampText = (updatedTimestamp: Date) =>
	moment(updatedTimestamp).utcOffset(0).format("YYYY-MM-DD HH:mm:ss");

const temporaryCommandSupportEmbedField = (
	command: string,
	severity: string,
	title: string,
	content: string,
...
const temporaryCommandEmbedField = (
	date: string,
	moderator: string,
	command: string,
	severity: string,
...
export const temporarycommandedit: CommandDefinition = {
	...
	executor: async (msg) => {
		...
		const supportOpsChannel = msg.guild.channels.resolve(
			Channels.SUPPORT_OPS,
		) as TextChannel | null;
		const hasPermittedRole = msg.member.roles.cache.some((role) =>
			permittedRoles.map((r) => r.toString()).includes(role.id),
		);
		...
		if (subCommand === "add") {
			...
			if (
				...
				!regexMatches.groups.severity
			) {
				return msg.channel.send({
					embeds: [
						missingInfoEmbed(
							"Add",
							`You need to provide the expected format to create a temporary command. Check \`${evokedCommand} help\` for more details.`,
						),
					],
				});
			}
			if (regexMatches.groups.title === "") {
				return msg.channel.send({
					embeds: [
						missingInfoEmbed(
							"Add",
							`You need to provide a non-empty title to create a temporary command. Check \`${evokedCommand} help\` for more details.`,
						),
					],
				});
			}
			if (regexMatches.groups.content === "") {
				return msg.channel.send({
					embeds: [
						missingInfoEmbed(
							"Add",
							`You need to provide a non-empty content to create a temporary command. Check \`${evokedCommand} help\` for more details.`,
						),
					],
				});
			}
			...
			if (searchResult.length !== 0) {
				return msg.channel.send({
					embeds: [alreadyExistsEmbed("Add", command)],
				});
			}
		...
		if (subCommand === "image") {
			...
			if (
				regexMatches === null ||
				!regexMatches.groups.command ||
				!regexMatches.groups.imageUrl
			) {
				return msg.channel.send({
					embeds: [
						missingInfoEmbed(
							"Configure image URL",
							`You need to provide the expected format to set the image for a temporary command. Check \`${evokedCommand} help\` for more details.`,
						),
					],
				});
			}
			...
			const temporaryCommandSearchResult = await TemporaryCommand.find({
				command,
			});
			if (temporaryCommandSearchResult.length === 0) {
				return msg.channel.send({
					embeds: [notFoundEmbed("Configure image URL", command)],
				});
			}
			...
			try {
				...
				return msg.channel.send({
					embeds: [failedEmbed("Configure image URL", command)],
				});
			}

			const { date, moderator, severity, title, content, lastUsed } =
				temporaryCommand;
			try {
				await modLogsChannel.send({
					embeds: [
				...
				await msg.channel.send({
					embeds: [noChannelEmbed("Configure image URL", "Mod Log")],
				});
			}
			...
			try {
				...
				await msg.channel.send({
					embeds: [noChannelEmbed("Configure image URL", "Support")],
				});
			}

			return msg.channel.send({
				embeds: [successEmbed("Configure image URL", command)],
			});
		}
		...
		if (!regexMatches || !regexMatches.groups || !regexMatches.groups.command) {
			const subCommandText =
				subCommand === "info" ? "show the info of" : subCommand;
			return msg.channel.send({
				embeds: [
					missingInfoEmbed(
						subCommand,
						`You need to provide the expected format to ${subCommandText} a temporary command. Check \`${evokedCommand} help\` for more details.`,
					),
				],
			});
		}
		...
		if (subCommand === "info") {
			...
			const { author } = msg;
			const { moderator, content, date, title, severity, imageUrl, lastUsed } =
				searchResult[0];
			return msg.channel.send({
				embeds: [
					infoEmbed(
		...
		if (subCommand === "delete") {
			...
			try {
				...
				if (!temporaryCommands || temporaryCommands.length !== 1) {
					return msg.channel.send({
						embeds: [notFoundEmbed("Delete", command)],
					});
				}
			} catch {
			...
			try {
				...
				await msg.channel.send({
					embeds: [noChannelEmbed("Delete", "Mod Log")],
				});
			}
			...
			try {
				...
				await msg.channel.send({
					embeds: [noChannelEmbed("Delete", "Support")],
				});
			}
