...
const listEmbed = (fields: EmbedField[], count: number) =>
	makeEmbed({
...
const temporaryCommandListEmbedField = (
	command: string,
	severity: string,
	title: string,
): EmbedField[] => [
...
export const temporarycommand: CommandDefinition = {
	...
	executor: async (msg) => {
		...
		if (subCommand === "show") {
			...
			try {
				...
				await modLogsChannel.send({
					embeds: [failedEmbed("Update last used", command)],
				});
			}
		...
		if (subCommand === "list") {
			const searchRegex =
				regexMatches && regexMatches.groups && regexMatches.groups.command
					? new RegExp(regexMatches.groups.command)
					: null;
			const temporaryCommands = searchRegex
				? await TemporaryCommand.find({ command: searchRegex })
				: await TemporaryCommand.find();

			const fields: EmbedField[] = temporaryCommands
				.sort((a, b) => a.command.localeCompare(b.command))
				.map((temporaryCommand) => {
			...
			if (temporaryCommands.length > 0) {
				await msg.channel.send({
					embeds: [listEmbed(fields, temporaryCommands.length)],
				});
				return;
			}
			if (searchRegex) {
				await msg.channel.send({
					embeds: [notFoundEmbed("List", regexMatches.groups.command)],
				});
				return;
			}
