...
const userBannedEmbed = (user: User, executor: User, reason: string) =>
	makeEmbed({
		color: Colors.Red,
		author: {
			name: `[BANNED] ${user.tag}`,
...
const userBannedIncompleteEmbed = (user: User) =>
	makeEmbed({
		color: Colors.Red,
		author: {
			name: `[BANNED] ${user.tag}`,
...
module.exports = {
	...
	executor: async (guildBanAdd) => {
		...
		do {
			Logger.debug(
				`Ban Handler - Finding Audit Log entry retries left: ${retryCount}`,
			);
		...
		if (!target) {
			await modLogsChannel.send({
				embeds: [noLogEmbed(guildBanAdd.user, guildBanAdd.guild.name)],
			});
			return;
		}
		if (target.id !== guildBanAdd.user.id) {
			await modLogsChannel.send({
				embeds: [userBannedIncompleteEmbed(guildBanAdd.user)],
			});
			return;
		}
		if (executor && ModLogsExclude.indexOf(executor.id) < 0) {
			await modLogsChannel.send({
				...
				embeds: [userBannedEmbed(guildBanAdd.user, executor, reason)],
			});
		}
	},
};
