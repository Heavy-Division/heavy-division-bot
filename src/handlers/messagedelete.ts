...
module.exports = {
	...
	executor: async (message) => {
		...
		const userLogsChannel = message.guild.channels.resolve(
			Channels.USER_LOGS,
		) as TextChannel | null;

		if (
			userLogsChannel &&
			!UserLogExclude.some((e) => e === message.author.id)
		) {
			const messageDeleteEmbed = makeEmbed({
				color: Colors.Red,
				thumbnail: {
					url: "https://cdn.discordapp.com/attachments/770835189419999262/779946282373873694/150-1509174_deleted-message-icon-sign-hd-png-download.png",
				},
				author: {
